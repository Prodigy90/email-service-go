.PHONY: build run dev test lint migrate-up migrate-down docker-up docker-down clean build-local push-local build-push
...
# =============================================================================
#
# CI builds linux/amd64 and tags as :latest
# Local builds linux/arm64 and tags as :arm64 (distinct from CI tags)
# =============================================================================

GHCR_IMAGE = ghcr.io/prodigy90/email-service-go
...
push-local:
	@docker push $(GHCR_IMAGE):arm64 && \
		(echo "Push complete!"; \
		 ...
		 echo "  kubectl -n wasbot-e2e rollout restart deployment/email-service") || \
		(echo ""; \
